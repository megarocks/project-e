(function (b) {
    ...
    b.extend(a.prototype, {initImage: function (c, d) {
        ...
        this.options.image.maxSize = c.max_size || this.options.image.maxSize || false;
        ...
        this.$file.off("file.error.ace").on("file.error.ace", function (f, d) {
            if (!c.on_error) {
                return
            }
            if (d.error_count.ext > 0 || d.error_count.mime > 0) {
                c.on_error(1)
            } else {
                if (d.error_count.size > 0) {
                    c.on_error(2)
                }
            }
        })
    }});
    a.defaults = b.extend({}, b.fn.editabletypes.abstractinput.defaults, {tpl: '<span><input type="hidden" /></span><span><input type="file" /></span>', inputclass: "", image: {style: "well", btn_choose: "Change Image", btn_change: null, no_icon: "fa fa-picture-o", thumbnail: "large"}});
...
(function (b) {
    var a = function (c) {
        ...
        try {
            ...
            f.type = "number";
            this.nativeUI = f.type === "number" && this.options.spinner.nativeUI === true
        } catch (d) {
        }
    };
    b.fn.editableutils.inherit(a, b.fn.editabletypes.abstractinput);
    b.extend(a.prototype, {initSpinner: function (c, d) {
        ...
        if (this.$input.is(":visible")) {
            ...
            if (!this.nativeUI) {
                ...
                var c = b.extend({value: e}, this.options.spinner);
                this.$input.ace_spinner(c)
            } else {
                this.$input.get(0).type = "number";
                var c = ["min", "max", "step"];
                for (var d = 0; d < c.length; d++) {
                    if (c[d] in this.options.spinner) {
                        this.$input.attr(c[d], this.options.spinner[c[d]])
                    }
                }
            }
        }
    ...
    a.defaults = b.extend({}, b.fn.editabletypes.abstractinput.defaults, {tpl: '<input type="text" />', inputclass: "", spinner: {min: 0, max: 100, step: 1, icon_up: "fa fa-plus", icon_down: "fa fa-minus", btn_up_class: "btn-success", btn_down_class: "btn-danger"}});
...
(function (b) {
    var a = function (c) {
        ...
        try {
            var f = document.createElement("INPUT");
            f.type = "range";
            this.nativeUI = f.type === "range" && this.options.slider.nativeUI === true
        } catch (d) {
        }
    };
    b.fn.editableutils.inherit(a, b.fn.editabletypes.abstractinput);
    b.extend(a.prototype, {initSlider: function (c, d) {
        ...
        if (this.$input.is(":visible")) {
            ...
            if (!this.nativeUI) {
                ...
                var d = b.extend(this.options.slider, {value: g, slide: function (h, i) {
                    var j = parseInt(i.value);
                    c.$input.val(j);
                    if (i.handle.firstChild == null) {
                        b(i.handle).prepend("<div class='tooltip top in' style='display:none; top:-38px; left:-5px;'><div class='tooltip-arrow'></div><div class='tooltip-inner'></div></div>")
                    }
                    b(i.handle.firstChild).show().children().eq(1).text(j)
                }});
                ...
                for (var f = 0; f < d.length; f++) {
                    if (d[f] in this.options.slider) {
                        this.$input[0][d[f]] = this.options.slider[d[f]]
                    }
                }
                var e = this.options.slider.width || 200;
                this.$input.parent().addClass("editable-slider").css("width", e + "px")
            }
        }
    }, value2html: function (d, c) {
    }, autosubmit: function () {
...
(function (b) {
    var a = function (c) {
        this.init("adate", c, a.defaults);
        this.initDate(c, a.defaults);
        this.nativeUI = false;
        try {
            var f = document.createElement("INPUT");
            f.type = "date";
            this.nativeUI = f.type === "date" && this.options.date.nativeUI === true
        } catch (d) {
        }
    };
    b.fn.editableutils.inherit(a, b.fn.editabletypes.abstractinput);
    b.extend(a.prototype, {initDate: function (c, d) {
        this.options.date = b.extend({}, d.date, c.date)
    }, render: function () {
    ...
    a.defaults = b.extend({}, b.fn.editabletypes.abstractinput.defaults, {tpl: '<div class="input-group input-group-compact"><input type="text" class="input-medium date" /><span class="input-group-addon"><i class="fa fa-calendar"></i></span></div>', date: {weekStart: 0, startView: 0, minViewMode: 0}});
    b.fn.editabletypes.adate = a
}(window.jQuery));